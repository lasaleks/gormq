...
func main() {
	...
	wg := sync.WaitGroup{}
	myutils.CreatePidFile("/tmp/gormqeExample2.pid")
	...
	chSend, err := gormq3.NewChannelPublisherWithAck(&wg, ctx_rmq, conn_rmq, CH_PUB_MSG)
	if err != nil {
		log.Panicln("connect rabbitmq", err)
	}
		...
		for {
			...
			case <-time.After(time.Second * 1):
				CH_PUB_MSG <- gormq3.MessageAmpq{
					Exchange:    "TestGORMQ2",
					Routing_key: fmt.Sprintf("TesGoRMQPub.gormq3.ack.%d", idx),
				}
				idx++
			}
